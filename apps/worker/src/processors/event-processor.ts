...
async function checkAndSetCooldown(redis: Redis, ruleId: string, userId: string, cooldownSeconds: number | null): Promise<boolean> {
  ...
  return async (job: Job<EventJobPayload>) => {
    ...
    try {
      ...
      const recentSession = await db.userSession.findFirst({
        where: { userId, lastSeenAt: { gte: new Date(Date.now() - 5 * 60 * 1000) } },
      });
      ...
      for (const result of matchedRules) {
        ...
        if (!rule) continue;
        const onCooldown = await checkAndSetCooldown(redis, result.ruleId, userId, rule.cooldownSeconds);
        if (!onCooldown) activeRules.push(result);
      }

      // Persist all rule executions (matched + unmatched)
      await Promise.all(ruleResults.map((result) =>
        db.ruleExecution.create({
          data: {
            ruleId: result.ruleId, eventId, userId, tenantId,
      ...
      const campaignActions = actions.filter((a) => a.type === 'ADD_TO_CAMPAIGN');
      ...
      for (const action of scoreActions) {
        ...
        const existing = await db.userEngagementScore.findUnique({ where: { userId } });
        if (field === 'score') {
          await db.userEngagementScore.upsert({
            where: { userId },
            update: { score: Math.min(100, Math.max(0, (existing?.score ?? 50) + increment)) },
            create: { userId, tenantId, score: Math.max(0, 50 + increment), fatigueScore: 0 },
          });
        } else if (field === 'fatigueScore') {
          await db.userEngagementScore.upsert({
            where: { userId },
            update: { fatigueScore: Math.min(1, Math.max(0, (existing?.fatigueScore ?? 0) + increment)) },
            create: { userId, tenantId, score: 50, fatigueScore: Math.max(0, increment) },
          });
        }
      }
      ...
      for (const action of campaignActions) {
        ...
        const decision = await db.engagementDecision.create({
          data: {
            ...
            aiGenerated: false, confidence: 1.0, priority: 1, scheduledFor,
          },
        });

        const deliveryQueue = getQueue(QUEUES.DELIVERIES_SCHEDULED);
        await deliveryQueue.add('schedule', {
          engagementDecisionId: decision.id, tenantId, userId, channel,
      ...
      if (isSuppressed || (sendActions.length === 0 && voiceActions.length === 0 && whatsappActions.length === 0 && pushActions.length === 0)) {
        ...
        await db.eventProcessingLog.create({
          data: { eventId, step: 'completed', status: 'ok', details: { suppressed: isSuppressed, actionCount: 0 } },
        });
        return;
      }
      ...
      const [aiFlag, voiceFlag, whatsappFlag, pushFlag] = await Promise.all([
        redis.get(REDIS_KEYS.featureFlag('ai_engagement_decisions', tenantId))
          .then((v) => v ?? redis.get(REDIS_KEYS.featureFlag('ai_engagement_decisions'))),
        redis.get(REDIS_KEYS.featureFlag('voice_campaigns', tenantId))
          .then((v) => v ?? redis.get(REDIS_KEYS.featureFlag('voice_campaigns'))),
        redis.get(REDIS_KEYS.featureFlag('whatsapp_campaigns', tenantId))
          .then((v) => v ?? redis.get(REDIS_KEYS.featureFlag('whatsapp_campaigns'))),
        redis.get(REDIS_KEYS.featureFlag('push_campaigns', tenantId))
          .then((v) => v ?? redis.get(REDIS_KEYS.featureFlag('push_campaigns'))),
      ]);
      ...
      if (aiFlag === '1') {
        try {
          ...
          console.error('[event-processor] AI consultation failed, proceeding without:', aiErr);
        }
      }
      ...
      for (const action of sendActions) {
        ...
        if (
          ...
          isQuietHours(user.timezone, channelPref.quietHoursStart, channelPref.quietHoursEnd)
        ) continue;

        // Validate recipient exists for the channel
        if (channel === 'email' && !user.email) continue;
        if ((channel === 'sms' || channel === 'voice' || channel === 'whatsapp') && !user.phone) continue;
        if (channel === 'push') {
        ...
        const scheduledFor = aiDecision?.schedulingOffsetMinutes
          ? new Date(Date.now() + aiDecision.schedulingOffsetMinutes * 60 * 1000)
          : new Date();
        ...
        const decision = await db.engagementDecision.create({
          data: {
            ...
            reasoning: {
              ...
              timingReason: aiDecision?.schedulingOffsetMinutes ? 'ai_optimized' : 'immediate',
              ...(templateId ? { templateId } : {}),
            },
            aiGenerated: aiDecision !== null, confidence: aiDecision?.channelConfidence ?? 1.0,
        ...
        await deliveryQueue.add(
          'schedule',
          { engagementDecisionId: decision.id, tenantId, userId, channel } satisfies DeliveryJobPayload,
      ...
      if (voiceFlag === '1') {
        for (const action of voiceActions) {
          ...
          if (
            ...
            isQuietHours(user.timezone, voicePref.quietHoursStart, voicePref.quietHoursEnd)
          ) continue;
          ...
          const voiceCall = await db.voiceCall.create({
            data: { voiceCampaignId: campaignId, tenantId, userId, phone: user.phone, status: 'queued' },
          });

          const voiceConfig = (campaign.voiceConfig || {}) as Record<string, unknown>;
          await voiceQueue.add('initiate', {
            voiceCallId: voiceCall.id, voiceCampaignId: campaignId, userId,
            phone: user.phone, script: campaign.script,
            languageCode: (voiceConfig['language'] as string) || 'es-ES',
            voiceGender: (voiceConfig['voice'] as 'male' | 'female') || 'female',
            dtmfConfig: campaign.dtmfConfig, attempt: 0,
          });
        }
      }
      ...
      if (whatsappFlag === '1') {
        ...
        for (const action of whatsappActions) {
          ...
          const waPref = preferences.find((p) => p.channel === 'whatsapp');
          if (
            waPref?.quietHoursStart != null &&
            waPref?.quietHoursEnd != null &&
            isQuietHours(user.timezone, waPref.quietHoursStart, waPref.quietHoursEnd)
          ) continue;
          ...
          await whatsappQueue.add(
            ...
            {
              whatsappCampaignId: campaignId, userId, phone: user.phone,
              body: campaign.body, headerType: campaign.headerType,
              headerValue: campaign.headerValue, footerText: campaign.footerText,
              buttons: campaign.buttons, attempt: 0,
            },
      ...
      if (pushFlag === '1') {
        ...
        for (const action of pushActions) {
          ...
          if (
            ...
            isQuietHours(user.timezone, pushPref.quietHoursStart, pushPref.quietHoursEnd)
          ) continue;
          ...
          await pushQueue.add(
            ...
            {
              pushCampaignId: campaignId, userId,
              title: campaign.title, body: campaign.body,
              imageUrl: campaign.imageUrl, actionUrl: campaign.actionUrl,
              priority: campaign.priority, attempt: 0,
            },
            { attempts: campaign.maxRetries + 1, backoff: { type: 'exponential', delay: 2000 } },
          );
        }
      ...
      await db.eventProcessingLog.create({
        data: {
          eventId, step: 'completed', status: 'ok',
          details: {
      ...
      await db.eventProcessingLog.create({
        data: { eventId, step: 'error', status: 'error', details: { error: message } },
      }).catch(() => {});
      throw err;
    }
  };
