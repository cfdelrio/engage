...
export function EmailCampaignList() {
  ...
  const handleDelete = async (id: string) => {
    try {
      setDeleting(true);
      const response = await apiFetch(`/v1/email-campaigns/${id}`, {
        method: "DELETE",
      });
  ...
  const handleStart = async (id: string) => {
    try {
      const response = await apiFetch(`/v1/email-campaigns/${id}/start`, {
        method: "POST",
      });
  ...
  const handlePause = async (id: string) => {
    try {
      const response = await apiFetch(`/v1/email-campaigns/${id}/pause`, {
        method: "POST",
      });
