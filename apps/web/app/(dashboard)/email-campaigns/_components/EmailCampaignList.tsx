...
export function EmailCampaignList() {
  ...
  const fetchCampaigns = useCallback(async () => {
    try {
      setLoading(true);
      const response = await fetch(`${API_URL}/v1/email-campaigns`, {
  ...
  const handleDelete = async (id: string) => {
    try {
      setDeleting(true);
      const response = await fetch(`${API_URL}/v1/email-campaigns/${id}`, {
        method: "DELETE",
        headers: { "x-api-key": apiKey },
      });
  ...
  const handleStart = async (id: string) => {
    try {
      const response = await fetch(
        `${API_URL}/v1/email-campaigns/${id}/start`,
        {
          method: "POST",
          headers: { "x-api-key": apiKey },
        },
  ...
  const handlePause = async (id: string) => {
    try {
      const response = await fetch(
        `${API_URL}/v1/email-campaigns/${id}/pause`,
        {
          method: "POST",
          headers: { "x-api-key": apiKey },
        },
