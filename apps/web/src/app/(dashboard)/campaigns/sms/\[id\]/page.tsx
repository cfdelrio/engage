...
export default function SmsCampaignDetailPage() {
  ...
  async function handleDelete() {
    ...
    try {
      setDeleting(true);
      const res = await fetch(`/api/v1/sms-campaigns/${campaignId}`, { method: 'DELETE' });
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          {campaign.status === 'draft' && (
            <>
              <Link href={`/campaigns/sms/${campaignId}/edit`}>
      ...
      <Card className="p-6">
        ...
        <div className="grid grid-cols-2 gap-6">
          <div>
            <label className="block text-xs text-slate-600 uppercase mb-2">From Number</label>
            <p className="text-slate-900">{campaign.fromNumber || 'Default'}</p>
          </div>
          <div>
      ...
      {/* Message Body */}
      <Card className="p-6">
        <h2 className="font-semibold text-slate-900 mb-4">Message</h2>
        <div className="bg-slate-50 p-4 rounded border border-slate-200">
          <p className="text-slate-900 whitespace-pre-wrap">{campaign.body}</p>
          <p className="text-xs text-slate-600 mt-4">
            {campaign.body.length} characters • {Math.ceil(campaign.body.length / 160)} SMS
          </p>
        </div>
      </Card>
