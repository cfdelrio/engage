...
import { Mail, MessageSquare, Bell, Phone, MessageCircle, TrendingUp } from 'lucide-react';
  ...
  const channelStats = [
    {
      ...
      openRate: Math.round((metrics.email.opened / metrics.email.delivered) * 100),
    },
    {
      name: 'SMS',
      icon: MessageSquare,
    ...
    },
    {
      ...
      openRate: Math.round((metrics.push.opened / metrics.push.delivered) * 100),
    },
    {
      name: 'Voice',
      icon: Phone,
    ...
    },
    {
      ...
      readRate: Math.round((metrics.whatsapp.read / metrics.whatsapp.delivered) * 100),
    },
  ];
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-slate-600 mt-2">Overview of your engagement campaigns</p>
      </div>
          ...
          return (
            <Card key={channel.name} className="p-6">
              ...
              <div className="space-y-3">
                <div>
                  <p className="text-2xl font-bold text-slate-900">{channel.sent.toLocaleString()}</p>
                  <p className="text-xs text-slate-500">Messages Sent</p>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-slate-600">Delivered:</span>
                  <span className="font-medium text-slate-900">{channel.delivered}</span>
                </div>
                {channel.openRate !== undefined && (
                  <div className="flex justify-between text-sm">
                    <span className="text-slate-600">Open Rate:</span>
                    <span className="font-medium text-slate-900">{channel.openRate}%</span>
                  </div>
                )}
                {channel.rate !== undefined && (
                  <div className="flex justify-between text-sm">
                    <span className="text-slate-600">Success Rate:</span>
                    <span className="font-medium text-slate-900">{channel.rate}%</span>
                  </div>
                )}
                {channel.readRate !== undefined && (
                  <div className="flex justify-between text-sm">
                    <span className="text-slate-600">Read Rate:</span>
                    <span className="font-medium text-slate-900">{channel.readRate}%</span>
                  </div>
                )}
              </div>
            </Card>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card className="p-6">
          <h2 className="text-lg font-semibold text-slate-900 mb-6">Messages Sent (7 days)</h2>
      ...
      <Card className="p-6">
        ...
        <div className="space-y-4">
          {[
            { campaign: 'Q2 Product Launch', type: 'Email', status: 'Active', count: '2,450/2,500' },
            { campaign: 'Weekend Reminder', type: 'Push', status: 'Active', count: '2,850/3,000' },
            { campaign: 'Support Follow-up', type: 'Voice', status: 'Paused', count: '320/450' },
            { campaign: 'Promotional SMS', type: 'SMS', status: 'Completed', count: '1,180/1,200' },
            { campaign: 'Customer Feedback', type: 'WhatsApp', status: 'Active', count: '600/800' },
          ].map((activity, idx) => (
            <div key={idx} className="flex items-center justify-between py-3 border-b border-slate-200 last:border-0">
              ...
              <div className="flex items-center gap-4">
                <div className="text-right">
                  <p className="text-sm font-medium text-slate-900">{activity.count}</p>
