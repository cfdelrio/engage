...
const eventStreamRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get(
    ...
    async (socket, request) => {
      ...
      const apiKey = (request.query as Record<string, string>)['apiKey'] ?? request.headers['x-api-key'];
      ...
      try {
        ...
        const keyHash = createHash('sha256').update(apiKey as string).digest('hex');
        ...
        if (cached) {
          ...
          await fastify.redis.setex(REDIS_KEYS.apiKeyCache(keyHash), 300, JSON.stringify({ tenantId }));
        }
      } catch {
        socket.close(4001, 'Unauthorized');
        return;
