...
const emailCampaignsRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      const emailQueue = new Queue(QUEUES.EMAIL_CAMPAIGN_DELIVERY, {
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      return {
        stats: {
          ...
          openRate:
            total > 0 ? Math.round(((stats["opened"] ?? 0) / total) * 100) : 0,
          clickRate:
            total > 0 ? Math.round(((stats["clicked"] ?? 0) / total) * 100) : 0,
          deliveryRate:
            total > 0
