...
export async function rateLimitApiKeyPlugin(app: FastifyInstance) {
  ...
  try {
    const result = await (
      redis as unknown as {
        script: (cmd: string, script: string) => Promise<string>;
      }
    ).script("load", RATE_LIMIT_LUA_SCRIPT);
  ...
  app.addHook(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      try {
        ...
        const counterKey = getRateLimitCounterKey(
          request.apiKeyHash,
          endpoint,
          windowStart,
        );
        ...
        // Calculate response
        const result = calculateRateLimitResponse(
          count,
          rule.limit,
          resetAt,
          now,
        ...
        logger.error(
          { error },
          "Unexpected error in rate limit plugin - allowing request",
        );
